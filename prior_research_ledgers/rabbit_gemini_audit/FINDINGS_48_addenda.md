...
- **The author used the legacy EVP-BytesToKey(SHA-256) KDF, not PBKDF2.**
  ...
  KDF.
- **The SalPhaseIon letter stream is fully accounted for.** Its 1,075 characters
  reconcile exactly against dbbi (91) + the matrixsumlist run (104) + faed (570)
  + lastwords (63) + thispassword (29) + 3 `z` separators + the shabef line (35)
  + the AES blob (128) + the enter run (40) + shabefanstoo (12). **No surplus.**
  Unlike the rabbit image, nothing is left over here.
...
| SalPhaseIon stream | **reconciles exactly, no surplus** |
...
# Addendum 32 — the artifact question closed: full-census sweep of every held object

Everything below was executed against the workspace files, live Wayback
captures, the live Decentraland content servers, and the 2026-09-09 Telegram
export. No step here cites community interpretation as evidence. Hashes are
recorded inline so any line can be re-derived.

## Provenance: the copies we hold are the original bytes

| object | captures compared | result |
|---|---|---|
| `follow_the_white_rabbit.png` | 2020-11-15, 2024-04-06, 2026-01-03 Wayback + community mirror | **byte-identical**, sha256 `5e8d84b88f8f8294…` all four |
| SalPhaseIon page | 2023-06-01 Wayback vs held copy | textareas byte-identical (1,075 / 1,792 chars), headings unchanged |
| rebus page `/theseedisplanted` | 2020-11-12, 2022-12-24, 2026-01-05, 2026-04-18 | 8 images + 1 form; differences are whitespace and the CSRF token only |

This closes, permanently, the caveat that ran since Addendum 1: *"this is a
mirrored file and may have been re-encoded."* It was not. The stego negatives
in Addenda 1 and 10 are statements about the original bytes.

## The grid is exactly the code, and the rabbit is drawn on top of it

Regenerated the full 14×14 grid from the string `gsmg.io/theseedisplanted`
alone (CCW-inward spiral, black/blue=1, markers = last bit of each character,
blue when ASCII is odd): **192 of 192 payload cells match exactly** — bits and
marker colours. The image carries zero information beyond the string.

Every deviating pixel was then located: **1,550 px** (the 62 sub-units of
Addendum 1, at 25 px each), forming — when rendered alone — two ears, a head
outline with eye and whiskers, and a filled body. **The body is the `#fefefe`
cell.** A white rabbit drawn on a white background, invisible without exact
pixel comparison. That single object consumes three standing items: the
unresolved `#fefefe` cell (known since 2020), phrase 6 *"it's in front of your
eyes but you're not seeing it"*, and "follow the **white** rabbit" — while the
body cell still sits at spiral index **163 (prime)**, byte 20 bit 3, the `n`
of "planted".

Obvious executions tested against both 5-block envelopes and Cosmic Duality,
both KDFs, raw and sha256-hex: `theseedisplated` (n deleted), full path
variant, `theseedispla0ted`, the `~` bit-flip, `163`, upper-case. **All null,
zero padding passes.** The marked-character instruction is not a direct
password.

## The 2026-09-09 export

- **HUNDRED FOURTY** stands as Addendum 26 resolved it (the 140-character
  genesis coinbase string). The creator's only direct statement on the
  spelling, #5960 (2021-03-01, in reply to a member noting the archaic form):
  *"Ancient spelling 😅. One of the many many typos."* Consistent with
  #1806/#3345/#3373. The number is data; the misspelling is voice.
- **The July 2026 AMA is now first-hand, with reply threading.** Three
  second-hand readings deflate: "hidden in a room with a hidden door" (#66903)
  is a direct reply to a member joking about physically searching for his
  laptop — it describes the laptop, not a puzzle room. "Working with many
  NOTES" (#66962) replies to a member waiting for the next "NOTE: that is a
  hint" — self-reference, not a cipher lead. "Give yourself yourself…"
  (#66961) sits inside the drug-philosophy stretch of the same session.
  Downgrade all three.
- **New and load-bearing:** *"IIFF I'm somehow still wrong… it's all still
  solvable with a few stable qubits"* (#66589) — the endgame is ECDLP-shaped,
  i.e. the final answer is a private key, and *"the btc is still available"*
  (#66595).

## Imports from the reference corpus (issue #106, 2026-08-22)

- **X2SH is officially dead.** Creator-dated 2021-02-12: *"#…# wasn't used."*
  This closes the Addendum 7 "three unconsumed pieces" list to two, and
  retroactively explains the Addendum 3 nulls.
- **"2nd way wasnt founded (shared)"** — the second door re-confirmed as
  unfound in February 2021, *before* SalPhaseIon was public. Independent
  confirmation of the Addendum 1 timeline argument.
- **A fourth open envelope exists:** salt `74c974e3f92e64b5`, 112 bytes,
  6 blocks — the corpus's "urlblob". Byte-unrelated to every known blob
  (verified: salt, length, ciphertext blocks all distinct). Not in any held
  image, page, or file (searched raw, base64, and hex). Not from any archived
  gsmg.io URL (full CDX census below). It entered the chat 2026-07-31
  (#67688, Kashin Kojin, claiming "hidden blobs in each phase"); the images
  he could have meant are provably clean, so the claim's *method* is wrong
  even if the blob is real. **Provenance unresolved — the one open object.**
- KDF refinement: the corpus reports Cosmic Duality decrypts under EVP-MD5
  with raw-digest passwords (author mixed openssl eras). Every sweep here
  already ran both KDFs, so nothing reopens — but future Cosmic work should
  treat MD5/raw-digest as the primary lane.

## The sibling session's sweeps, accounted

`src1616` (line-1616 fields): 31,248 decrypts, null (126 pad-1 vs 122.1
expected, nothing above pad 1). `arch3`/`arch4`: ~34M trials, null. `allwin`:
11.8M, pad histogram exactly at chance. `seven.py` (5,040 orderings + weaves
of the seven phase-3 parts) and `h140` (47,598 140-char windows) **died with
their session before running** — the loss is small (the families overlap
`final_block.py`'s phrase permutations), but they are not completed nulls.

## Decentraland: closed

The scene is still live on the content servers (entity
`QmRK2YoL…zN51v`, deployed 2020-02-20, owner `0x5D80…b546b`, both parcels).
The webpack bundle embeds its source map; the **original TypeScript is
recovered in full**: one clickable cube playing `sounds/puzzlepiece.mp3`, eight
decorative cubes, a sign "GSMG.IO 5 BTC PUZZLE CHALLENGE". No doors, no URLs,
no additional entities, no second sound. The parcel's only payload was the
audio, consumed in 2020. Live MP3 sha256 `ef17a96d…dfee`, 212,031 B.

The MP3 itself: ID3v2.2 of 4,086 B — `TSS` "Logic Pro X 10.4.1" (provenance:
he authored the sound), iTunes `iTunNORM`/`iTunSMPB` comment frames, zero
bytes between tag and first audio frame, no trailing data, no embedded
signatures. Fully accounted.

## Derivative artifacts: not puzzle material

`audio_seg0/1/2.wav` are **synthesized tones** — the prior session's `audio.py`
mapping the dbbi/faed digit streams to pitches (22050 Hz mono, LSB balance
0.502 = generated sine tones). `audio_COSMIC_raw.wav` is ciphertext bytes
rendered as PCM. All generated PNGs (cipher_bits/bytes, turtle_*, spectrogram_*)
are structurally clean renderings. **Nobody should chase any of these again.**

## Full bit-plane sweeps: all null

Poster (1048×1556) and grid (350×350): 4 channels × low bit-planes × row/column
scan × both bit orders — no `Salted__`, `U2FsdGVkX1`, `http`, `gsmg` anywhere.
The eight rebus images: **first bit-plane scan ever performed on them** (the
prior work measured layout only) — null, uniform alpha.

## The CDX census: every archived page is now identified

All 403 captures of gsmg.io classify into: the SPA shell (~12 KB, 200-for-
anything), the known real puzzle pages (rebus, SalPhaseIon, phase2), the eight
rebus images + grid image, platform assets (`app.css`, `app.js`, fonts,
favicons), `/register` referral pages, ~60 `/shared/<hex>` marketing shares
(2020-2021, fetched and read: trading-bot content, zero puzzle material),
`robots.txt` (empty), lodash-fragment scanner noise, and solver-guessed paths
returning the shell. **No unaccounted real page exists in the archive**, and
the urlblob salt appears in no archived URL.

## Where this leaves the door

Three doors remain, none of them objects waiting in the material:

1. **The marked-character operation** — the invisible rabbit on the `n` of
   "planted", prime index 163, triple-hinted (primes / zeroed-out / ASCII 127),
   never executed by anyone in seven years. Blocked only by gsmg.io serving a
   9-byte placeholder since July 2026.
2. **The personal door** — the creator's own framing (hidden laptop, close
   friends, "some of you can find me").
3. **The urlblob's provenance** — a real envelope with no source.

Everything else that can be measured, has been.

---

# Addendum 33 — the January 2020 "typo fix": tested, and it never happened

## What was tested

Creator message #1726 (2020-01-27): asked whether, besides givetit, there were
other spelling errors in 3.2, he answered *"Might have been fixed fyi 👆"* —
read as: the site's typo may have been live-patched in January 2020. If a fix
had been deployed to any *encrypted* stage, the ciphertext bytes on the page
would have changed. Wayback now settles it:

| page | captures compared | result |
|---|---|---|
| phase 2/3 page (`/choiceisanillusion…i wroteitmyself`) | 2020-11-12 vs 2022-12-23 (earliest two) | **ciphertexts byte-identical** — phase 2 salt `06286612…` 656 B, phase 3 salt `9fbc451d…` 4,096 B; visible text **zero diff**; CDX digest unchanged through 2026-04 |
| `/puzzle` (the poster) | 2020-11-12, 2023-01-08, 2026-01-04, 2026-04-18 | **same CDX digest all four**; the 2020 capture decompresses to **exactly our `puzzle.png`** (sha256 `38125bbd…`) |

The `/puzzle` finding also closes the poster's provenance caveat for good:
the community mirror is the original served bytes, and they never changed on
the server through 2026.

## Why the typo was provably never fixed

1. "giveit"/"givetit" appears in **no page's visible text** — the typo is
   inside phase 3's *encrypted* plaintext. Fixing it meant re-encrypting, which
   would change the ciphertext. The phase 3 ciphertext is byte-stable across
   every capture that exists.
2. The blob we hold still decrypts with the givetit-chain password — the
   community derived that requirement *from this same ciphertext*. A patched
   blob would take the corrected chain.
3. The password hash (`250f3772…`) was published in 2019; a patch would have
   broken it and been noticed within days.
4. He said so himself, effectively: he tested "the entire puzzle 3 times with
   the same string which had the typo in it" (#871) and issued the fix as a
   *hint*, not a site change.

## The residual gap, stated honestly

No capture of the phase 2/3 page exists between launch (2019-04) and
2020-11-12, so a re-encryption *in that window* cannot be excluded by capture
evidence alone — but points 2–4 exclude it independently. Whatever "might
have been fixed" referred to, nothing in the encrypted chain ever was, and
every decrypt in this document has been run against the original, unpatched
bytes throughout.

**Bonus closure:** the 3.2 plaintext's misspellings (fourty, waisting,
throphies) are inside the 3.2 envelope, which is inside phase 3's plaintext —
two layers below a ciphertext that never changed. They are original
authorship, not patch residue.

---

# Addendum 34 — RETRACTION: the "invisible white rabbit" reading in Addendum 32 is wrong

Addendum 32 claims the deviation pixels form *"a white rabbit drawn on a white
background, invisible"* whose *"body is the `#fefefe` cell."* **Both parts are
false, and the group knew every true component years ago.** This is the
correction.

## The pixels, re-measured

| measurement | value |
|---|---|
| `#fefefe` cell (grid PNG) | **625/625 px of (254,254,254) — pure. Zero rabbit pixels inside it.** |
| rabbit line art (grid PNG) | **925 px, all black (0,0,0)** — a black-outlined rabbit, visible |
| cells containing rabbit art | (7,7), (7,8), (8,7), (8,8), (8,9), (8,10), (9,7) — seven cells centred on the grid |
| `#fefefe` cell position | (8,5) — **three cells left of the rabbit, no overlap** |

The rabbit is black line art, not white-on-white. The `#fefefe` cell stands
*beside* it. The error came from reading a deviation map in which both the
black art and the off-white cell printed as identical `X` marks, then
narrating them into one object. Addendum 1's earlier "the rabbit occupies the
four surplus centre cells" is also imprecise — the art spans seven cells,
including payload cells.

One minor real observation survives this re-measurement: at the `#fefefe`
cell, the **poster and the grid PNG differ** — the poster's version of that
cell contains 490 black + 28 blue + 268 white pixels (a whisker crossing it),
the grid PNG's contains none. Sub-cell rendering difference; the two images
agree at every cell centre (Addendum 20). Not elevated to a finding.

## What the group already knew — all of it

From the 2026-09-09 export, 186 `fefefe` mentions:

- **Adjacency to the rabbit**: *"the white square which is fefefe color instead
  of ffffff? the one in front of rabbit`s face"* — x7x7x7x6, **May 2020**
  (#3715). Repeated: "in front of rabbit eyes" (#12964, 2023), "The rabbit is
  looking at it" (#16655, 2023), "right in front of rabbits eyes yet all seem
  to forget this fact" (#27567, 2024), "Rabbit is looking at this fucking
  FEFEFE" (#47392, 2025).
- **The 'n' of planted mapping**: *"we have it as 'n' in theseedisplanted and
  recognized it for long time"* — Worldmaker, #51055 (2025-10-25).
- **The spiral index**: *"fefefe is 163 or 164 ?!"* — sol, #66003 (2026-06-28).
- **Phrase 6 connected to it**: *"#103 block is in front of rabbit eyes which
  also aligns with: ... its in front of your eyes but youre not seeing it"* —
  X, #37553 (2025-03-31). This documents Addendum 18's phrase-6 assignment as
  prior art, not ours.
- **Pixel-level work inside that cell**: Dasharo's full census #56187
  (2026-01-11) and MrNobody's *"the rabbit drawing crosses its centre"*
  #68280 (2026-08-08, of the poster).
- **Grid predetermined by the URL**: ArchOptic, #10992 (2023-08-22): *"The
  data in the original 14x14 matrix is predetermined by the URL encoded into
  it with one exception: fefefe."* Even our bit-exact regeneration was a
  verification of a known statement.

## What actually remains ours, after this correction

The rabbit leaves the discovery list entirely. The session's surviving
contributions are the verification/closure class: byte-stable provenance
(Wayback, poster, Decentraland, the typo-fix test), the CDX census, the
creator-edit statistics, and the corrected cell census (925 px over seven
cells). The lesson is the same one Addendum 30 recorded: a striking pattern,
verified rigorously at the component level, with the *inference* drawn from it
never checked against either the pixels or the seven-year chat record — which
held the answer all along.

---

# Addendum 35 — phase 3.2 read as a document, not a wordlist

Reconstructed from primary bytes (2020 Wayback phase-3 ciphertext → known
passwords → `corpus/phase32_raw.txt`, 2,422 B). The document's true layout:

| offset | size | content |
|---|---|---|
| 0 | 332 B | preamble: greeting, *"...am I here? Wake up, you..."*, *"beautiful strategic position"*, *"One for one, four for one"* |
| 443 | 1,547 B | EBCDIC blob (3.2.1); the 7-char run `:>/`>/[` at 681 is ordinary ciphertext, visible in the community's pasted decode — not an anomaly |
| 1,990 | 149 B | VIC digit string |
| 2,143 | 145 B | *"Raising the stakes…"* + the VIC board sentence |
| 2,292 | 128 B | terminal envelope (5 AES blocks) |

## The document's grammar: every key is stated next to its lock

Each layer's method or key appears in the document's own plaintext, adjacent
to the layer it opens: the Beaufort cipher is *named* ("beautiful strategic
position"), its **key is quoted** ("Wake up, you…" → THEMATRIXHASYOU), the code
page is spelled out ("One for one, four for one" = 1-4-1-1 = EBCDIC 1141 — the
same line's "one…four" also giving the VIC straddle rows), and the VIC board
follows its own digits. By this grammar the terminal envelope's key is a
literal of this document — and the only unconsumed literals are the first
sentence of the VIC paragraph and the speech's swapped nouns. A 186-candidate
sweep of every such literal (KEYNOTE, CIAO BELLA O, 1141, the board phrase,
23/16/7 forms, all casings, raw+sha256, both KDFs, all three open envelopes):
**zero pad≥4 hits.** Either the literal needs one of the document's own
transforms applied (VIC-board encoding → digit-string → a-i-o letters is the
one taught transformation never applied to a phrase), or the grammar breaks at
the last step — which phrase 7 ("very last step is a true giveaway") argues
against.

## 23/16/7 is an inventory, and the 7 is identifiable

The film's line is 16 female + 7 male = 23. The creator's edit keeps the
arithmetic — "OVER TWENTY-THREE CIPHERS, SIXTEEN ENCRYPTIONS AND OR SEVEN
INTERTWINED PASSWORDS" — and the puzzle contains exactly one "seven
intertwined passwords": **the phase-3 password, seven parts concatenated.**
It was already spent by the time this text is readable, which makes the line
retrospective — a curriculum summary ("select from what you used"), not a
forward instruction. This retires the 23/16 numerology and is consistent with
his 2023 "the hardest part is done."

## X2SH: two primary sources disagree, and the sentence needs it

"As wide as the first one seen": the only 10-wide boards in the chain are the
QWERTY row (inside the Q riddle) and **the X2SH block itself** — the rabbit
matrix is 14, the chess board 8. The corpus's 2021 screenshot ("#…# wasn't
used") kills it; but in #8569 (2023-06-10), asked directly whether Norton's
theorem and X2SH are pointless, the creator answered **"Can't say anything
about this"** — in a message where he *did* answer "Correct" and "No" to the
two other questions. A free confirmation of uselessness was available and
declined. X2SH's status is genuinely unresolved, and the corpus's paraphrase
is unverified (the screenshot itself is in the outstanding request). Note
also the ambiguity of "#…# wasn't used": it may name the `#` wrappers, not
the block.

## The residue pattern

Each stage mixes symbol systems and leaves exactly one unexplained element:
the poster (4 working colours + the `#fefefe` cell), 3.2 (5 systems + one
sentence), the SalPhaseIon page (4 systems + dbbi/faed). Three residues,
three live objects — the same three this record keeps arriving at.

## Also settled this pass

- **lastwords/thispassword decode exactly from our copy** (a=1..i=9, o=0 →
  digits → hex → ASCII). The corpus's "not reproduced" note is a defect in
  their committed copy or method, not a crack in the decode.
- The 11×11 grid of dbbi-with-reinserted-zeros (91+π(121)=121=11²): printed,
  no visual structure; page-convention decode of both zero-reinserted strings
  (121 and 695 digits): noise. Recorded as nulls.

---

# Addendum 36 — the terminal envelope campaign: every principled axis, closed

Target: `b45a5e3d827593ca`, 5 blocks, the end of the 3.2 document. Every family
below ran against all three open envelopes, both KDFs, under the full gate
(pad≥4 + printable + hex/WIF/base58-text keys + secp256k1 against both prize
hash160s). Self-test: the gate reproduces keys planted as raw bytes, hex, WIF
and base64.

| axis | candidates | result |
|---|---:|---|
| document-grammar literals (KEYNOTE, CIAO BELLA O, 1141, board phrase, 23/16/7 forms…) | 186 | null |
| twin cross-keys (each envelope keyed by the others' b64/ct/salt, raw+sha) | 133 | null |
| document self-hashes as hex-passwords (3.2 plaintext, speech, checkerboard, VIC digits, EBCDIC blob…) incl. double-sha | ~60 | null |
| taught transforms of phrases/sentence (VIC-encode→digits→a-i-o; Beaufort-encode; EBCDIC-encode) | ~70 | null |
| halved-message composites, half/betterhalf prefixes | ~60 | null |
| creator post-2023 phrases (better half is hungry, 2 hours max, …), identity (SoWut, Jrk Bgrt), episode 3.5 | ~60 | null |
| film choice-scene exact lines (architect_scene.txt) | ~30 | null |
| **envelope halves** — the terminal b64 is two 64-char lines, a literal "half and better half"; each line, its sha, crosswise halves of the twins | ~30 | null |
| z/0/o/space-joined seven phrases (salph separator convention) | ~12 | null |
| union family re-gate (runall's 4,056 strings × 4 forms) | 24,336 decrypts | **zero pad≥4** |
| **raw-key/IV mode** (the creator's scare-quoted "'decrypt'"): sha256-objects as keys × 8 IV schemes; sliding 32-B windows of speech/checkerboard/document/stream; hex-digest-string keys | 20,379 | zero valid padding |
| **zeroed.py completed** (the 45%-dead sweep): 5 segments × zero-mappings {A,B,DEL} × z∈∅..2 letters × bases 10/9/8 × big/little → 1,195 readings, **325,006 window key-checks** against the on-chain oracle, plus every reading as an envelope password | 1,195 readings | **null — completed** |

Also closed this pass: VIC-decode of dbbi (91) and faed (570) through the
validated 3.2.2 board — noise, no English (the corpus's "last unexplained
step" remains unexplained; the board is not its mechanism).

## Where that leaves the terminal envelope

Roughly 46,000 new trials on top of the audit's ~6M and the corpus's 2.5B,
covering every axis derivable from the puzzle's own material: the document's
key-adjacency grammar, the twins symmetry, the taught transforms, the
creator's post-2023 vocabulary, his identity, the film's choice scene, raw-key
mode, and the completed zeroed-readings space. The envelope is not openable by
any construction we can justify from the record. Consistent with the
creator's own framing, what opens it is one of: a reading of dbbi/faed nobody
has, personal knowledge (his one labelled hint), or material behind the dead
site. Brute force as a method is finished here — the remaining entropy is not
computational.

---

# Addendum 37 — three doors, worked

## Behind the dead site: the client side is now measured shut

The site was a Vue SPA + Laravel backend. Wayback holds 35 captures of
`app.js` including a **2020-07-22 source map with full `sourcesContent`** —
the entire front-end source tree (229 files), recovered here. Findings:

- `router/routes.js` (complete source extracted): the SPA knows exactly one
  puzzle route, `/puzzle`; everything else is the trading app (dashboard,
  markets, settings, admin). **No hidden routes.**
- `pages/puzzle/index.vue` is an empty shell (`__vue_script__ = null`) — the
  real puzzle pages were **server-rendered** (consistent with the CSRF-token
  POST form on the rebus page). The catch-all `*` route explains the
  200-for-everything behavior that misled Addendum 29.
- Zero puzzle strings (`salph`, `theseedis`, `89727c`, `yinyang`, `cosmic`,
  `architect`…) anywhere in any of the 229 sources.

So "material behind the dead site" that is client-side does not exist. What
remains behind the site is the set of server-rendered puzzle pages that were
never captured — gone unless the site returns.

## dbbi/faed: the first hard measurement, and the two streams are not alike

| | n | IC (raw) | entropy | dominant symbols |
|---|---:|---:|---:|---|
| dbbi | 91 | **0.1509** | 2.88 / 3.17 bits | **b 27.5%, e 19.8%** (47% combined) |
| faed | 570 | 0.1181 | 3.12 / 3.17 bits | flat; only g lifts (18.8%) |

Uniform-9 reference IC = 0.1111. **dbbi is structured — its IC is far above
uniform and its distribution is heavily skewed; faed is essentially uniform.**
This is the first recorded evidence that the two streams play different roles
(key-like vs. well-mixed), which independently supports the community's
instinct to key faed by dbbi — while killing any theory that both are the same
cipher over plaintext. Noted, unexploited: an English letter→digit mod-9
mapping predicts a peak class at ~22% (e,n,w); dbbi's e-class sits at 19.8%,
but its b-class at 27.5% exceeds any natural English class — frequency alone
cannot pin the mapping.

New readings, all null (printable 0.27–0.42, no prize windows, no envelope
pads): a=0..i=8 base-9 (the "First or zero" indexing hint applied to the
letter code — legal base-9 for the first time, since Python's `int()` rejects
digit 9 and the old zeroed sweep silently skipped base-9/8 families containing
i/g); bijective base-9; zero-reinserted 121/695 under both mappings; 11×11
column and spiral reads; 695 as 5×139 and 139×5 transposes; every reading also
as an envelope password.

## The personal door

Not advanced. The company terms PDF is CID-encoded boilerplate (no ToUnicode
— unextractable, and the corporate entity is the wrong object anyway). The
labelled hint (close friends, "find me", the hidden laptop) remains
non-computable from here. Identity-family passwords (SoWut, Jrk Bgrt) tested
in Addendum 36 — null.

---

# Addendum 38 — every hint vs every finding: the missed list, and two corrections

## Corrections to this session's own claims

1. **"The live Decentraland audio is noise" — WRONG.** My decode forced mono
   mixdown, which cancels the payload channel. The corpus's verified
   procedure is `mono = left − right`; reproduced here independently: the
   difference channel has spectral flatness **0.023** (highly structured — a
   ~65 Hz odd-harmonic drone) versus 0.84 in either raw channel, and carries
   the known HASHTHETEXT spectrogram. Same error class as Addendum 30's.
2. **"The scene was redeployed 2020-02-20" — dead.** The community downloaded
   the identical file in 2021, 2024 and 2025; the corpus audit (2026-08-03)
   verifies the same CID and sha256 as ours. The entity timestamp is a
   content-server artifact, not a creator action.

## Hints never executed by anyone (chat, corpus, or this audit)

| hint | date | status |
|---|---|---|
| *"Focussing on the **theory of everything** is also still a valid path"* | 2023-01-12 | an entire declared-valid path; absent from the corpus's 65 sealed families and every sweep here. Never walked. |
| *"Hush hush"* (Roses sign-off) | 2020-01-14 | never interpreted |
| *"And Oen"* (reply to "Neo is anagram for One") | 2020-04-08 | never worked |
| *"I prefer **superpositions**"* | 2024-03-26 | never resolved; candidate reading never made: the `#fefefe` cell is a literal superposition (reads 0 in the decode, marked 1 by position) |
| *"**ASCII 127**"* | 2024-11-29 | never resolved (DEL; deleted-`n` executions null) |
| *"the hints state the path pretty obvious"* | 2020-08-12 | never decomposed into which hints state which path |
| *"one team managed to find something… others haven't yet found"* | 2020-04-08 | the thing found was never identified |

## Readings never weighed

- **"OVER twenty-three"** — the inventory is *more than* 23, so Addendum 35's
  16+7=23 is a subset, not the total. Adjusts the inventory reading, doesn't
  break it.
- **"a TEMPORARY dissemination"** — "temporary" was never weighed: the
  weaves permute-and-keep; the word suggests spread-then-restore semantics
  during a derivation, not in the final string.
- **The 'decrypt' scare quotes as null hypothesis** — the envelopes may
  contain nothing (author trolling); explains seven years economically,
  unfalsifiable without the key. Belongs on the table alongside real-content
  readings.
- **"First or zero"** — tested only as a=1/a=0 letter mappings; its actual
  referent (which piece, or how to index) remains unidentified.

## Verifications still open

- The hint-edit diff against **public** archives (2019–2020 Reddit threads,
  Bitcointalk) — passive, asks nothing of anyone, still not done.
- The exact wording of the 2021-02-12 "#…# wasn't used" screenshot — X2SH's
  status (dead vs. wrappers-only) hinges on a paraphrase nobody has verified.

## What the sweep confirmed as genuinely closed

Infrared (→ the inaudible-in-mixdown channel, consumed by the corpus),
HASHTHETEXT (authenticated, and its `shabef`-style substitution into the
envelope pipeline tested by the corpus — 96 pairs, no token-6/7 hit), the
audio end to end, the typo, the provenance set, the counting inventory, and
the 479. The residue after this pass is the table above — six unexecuted
hints, four unweighed readings, two unverified texts — plus the standing
three: dbbi/faed's mechanism, the personal door, the dead site's server side.

---

# Addendum 39 — the theory-of-everything path, walked

## The primary context, recovered

The full thread (Dec 2022 – Jan 2023): Alex asked *"What is easier? To find
the answer to this puzzle or to discover the physics Theory of Everything?"*
(#8308). The creator: *"not sure how long it would take to discover that
theory 🤔"* (#8309), then on 2023-01-12: *"Focussing on the theory of
everything is also still a valid path to reaching the private key"* (#8354),
*"At least higher than sqrt(-1)"* (#8353, to "chances < 2⁻²⁵⁶?"), *"An
imaginary puzzle would [have an imaginary answer]"* (#8356) — and **replied
to the original TOE question with a GIF (#8358)**: the **"YOU DON'T SAY"**
sarcasm meme (7 frames, no hidden content, verified by frame-diff).

**Two corrections to Addendum 4** ("the image binary is not in the export"):
both images ARE in the 2026-09-09 export — barrystyle's 377×499 PNG
(`files/image_2022-12-11_07-09-14.png`) and the creator's GIF
(`video_files/animation (4).gif.mp4`). The barrystyle image is identified:
the **Time-Life "Cosmic Duality" cover, "Seekers of the Unknown" series** —
matching the audit's inference. Its "scary specific" content remains to be
mined once the puzzle's endgame is known.

## The meme's reading

Replying to "which is easier: the puzzle or the TOE?" with *you-don't-say*
sarcasm says the two tasks are **the same task** — solving the puzzle *is*
discovering its theory of everything. "ALSO still a valid path" + the
imaginary-number exchange (the answer is real, not imaginary) together say:
one real unifying reading reaches the key. That is the same statement the
yinyang-gate structure makes; the TOE hint reinforces the gate rather than
opening a parallel lane.

## The community's readings, inventoried (85 messages, 2023–2026)

42/Hitchhiker's (incl. chr(42)='*'; π 3.14→3×14=42); yin-yang philosophy;
the four forces unified; "stack everything we've used"; the 24 primes (=
the 479); PNG "physical" dimensions/pHYs; other puzzles (1flamen6, Cicada);
Philip Glass's *Einstein on the Beach*; Paul Hague's *Theory of Everything*;
ECC ±y duality; "TOE makes the correct seed" (2026); and RB's counter-reading
— *"a joke, not a hint… the TOE holds the answer to any possible question, so
it also has the solution to this puzzle."* No reading was ever converted
into a mechanism.

## Walked mechanically — all null

- the **everything-stack**: all 29 authenticated stage answers in chain
  order (URL, rebus phrase, seven phase-3 parts, digests, riddle answers,
  Beaufort key, VIC board, seven phrases, balances, 42) — concatenated,
  z-joined, sha256'd, double-sha256'd;
- **four-into-one** unifications of the stage passwords;
- the **42-family** (19 literals: fortytwo, the answer to life the universe
  and everything, deepthought, `*`, chr(42), 6×9…);
- TOE self-referentials (toe, theoryofeverything, imaginarytime, sqrt(-1));
- **mod-4 ("one for one, four for one") quaternary readings** of dbbi, faed
  and both zero-reinserted strings as 256-bit key windows against the prize
  oracle.

432 envelope trials + 4 key windows: **zero hits.** The TOE path's mechanism,
if it is not the unification-reading itself, remains un-found — consistent
with everything else pointing at the reading, not the computation.

---

# Addendum 40 — the site is ALIVE: Addendum 30 overturned, and the door batteries, run live

## The correction that unblocks everything

Addendum 30 concluded the July 2026 site change had killed the puzzle's
oracles ("the site does not discriminate"). **False — it was tested only
against a hash-path that never existed.** Verified live on 2026-09-09:

| live path | response |
|---|---|
| `/theseedisplanted` | **200, 832 B, real HTML** (rebus page; 8 images + hidden form) |
| `/puzzle` | **200, 29,931 B — the poster PNG, byte-identical to our copy** |
| `/choiceisanillusion…iwroteitmyself` | **200, 9,207 B** (phase 2/3 page, textareas intact) |
| `/89727c598b9cd1cf…` (SalPhaseIon) | **200, 4,536 B** (both textareas byte-lengths unchanged) |
| `img/follow_the_white_rabbit.png`, logos, favicon | **200, real PNGs** (grid image byte-identical) |
| `img/logo_GSMG_restored.png` | live; a **blue→gold whole-globe recolor** of the logo (8,128 vs 8,590 B, same 500×620), served by 2026-08-19 |
| **POST `/phase1verification`** | **wrong password → 404 "Hello :-)"; the real phase-1 password → 302 → the phase-2 page. The interactive gate still validates.** |

"Hello :-)" is merely the 404 handler. The live rebus page also carries an
HTML comment the 2020–2022 captures lack — *"Nice to see you around! Good
luck little bunny hunter ;)"* — added between Dec 2022 and Jan 2026.

## The door batteries, run against the live oracle (all null)

Control first: `sha256("GSMGIO5BTCPUZZLECHALLENGE"+address)` → **live
SalPhaseIon page** — the digest-to-URL convention confirmed working; the
method is sound. Then, every non-404 would have been a door:

- **modified-seed URLs**: every single-character deletion and substitution at
  the marked `n` (all 36 alphanumerics), 0/o/~/n insertions, case forms;
- **prime-position zeroing of the seed** ("some characters… zeroed out" taken
  plural, the 121/695 convention): insert/delete/substitute `o` and `0` at
  all prime positions, on the path form and the full-URL form — as paths, as
  gate passwords, and as digests;
- **door-word paths** (34: door, nest, rabbithole, hush, zion, …);
- **digest-URLs**: the SalPhaseIon recipe (`UPPER(text)+address`, both
  orders) over 30 candidate texts — HASHTHETEXT, the caption, the seed, the
  poem, half-and-better-half, the checkerboard message, phrases — plus bare
  digests;
- **gate-password hunt**: 22 chain answers POSTed to the live endpoint,
  seeking a second accepted password;
- **endpoint namespace** (16 verification variants) and an **/img/
  dictionary** (52 names): empty.

## Standing conclusions

1. The door is not any single-character modification of the seed, not a
   door-word, not a digest of any known material, not a second password from
   the obvious list — on a server that *does* discriminate. These are now
   hard negatives, not blocked tests.
2. The site's survival makes every URL/password hypothesis cheaply testable
   from now on. The three-year premise "the door is blocked behind the dead
   site" is dead; what remains is the same reading problem as everywhere
   else.
3. The recolored "restored" logo and the bunny-hunter comment show the
   puzzle's static surface was touched in the late era (2022–2026). A live
   server that still validates the gate, carrying quiet late-era edits, is
   itself consistent with a creator who expects someone to keep knocking.

---

# Addendum 41 — the "decorative cubes" were a drawing: a gate, with the key at its base

Correction to Addendum 32 (and the audit's Decentraland closure), which
called cube2–cube9 "eight decorative cubes." The recovered source gives every
cube x=8 — a flat 2D figure stood up in 3D. Plotted at **true aspect**
(2.75 wide × 6.5 tall — the figure is narrow and tall, not the wide "Roman
arch" an unscaled ASCII suggests):

```
7.5      X
7.0   X     X
6.0  X        X
5.0            X      <- right side continues below the arch foot
4.0      X
3.0      X
2.0      ·             <- gap
1.0      X             <- the AUDIO cube
```

**Measured facts:** the five upper cubes form a symmetric peaked structure
(apex-centred, matched-y flanking pairs). Against random placement from the
same coordinate menu, that symmetry occurs **0.63%** of the time — deliberate
(~1-in-160; humans make symmetric things on purpose). The right side extends
below the left; a detached short column sits below-centre; the clickable
audio cube sits on the centre-line at the base, separated by a gap.

**Readings, ranked by support — REVISED 2026-09-09 (v3) after the user
asked about "the question mark":**

The strongest reading is neither gate nor rabbit but a **question mark** —
community-attested and structurally diagnostic. In the export's final active
days (2026-09-07/08), E calls it *"the reverse question mark where we got the
file"* and proposes checking it from all four sides (#71183, #71188, #71192);
Cereal Killer confirms it reads as a question mark from multiple angles
(#71193). The diagnostic feature: the audio cube is **detached below a
two-unit gap** — the defining dot of a "?" (a gate or figure does not float
its key). The structure maps cleanly: hooked top (the five symmetric cubes),
right-side descent curling inward (cube7), center tail (cubes 8–9), gap at
y=2, dot at y=1 = the audio cube playing `HASHTHETEXT`.

1. **A question mark with its key at the dot** — the puzzle drawn as a
   question, the answer-to-the-question at its point. Resonances: "Are you
   really looking for just the btc…?"; X2SH's H = the Answer × −1; 42 as the
   answer to everything (the TOE thread).
2. Gate/doorway — superseded; the arch part is simply the hook of the "?".
3. Rabbit mid-jump — soft, unchanged (Addendum 34 caveat applies).

Also recovered from the same final-days thread: **Elite Booter's "Hush hush
= Hash Hash"** (#71184) — a reading of the never-interpreted Roses sign-off
that fits both the doubling signature and the hashing mechanism; and **E's
half-and-better-half reading of the SalPhaseIon URL** (#71180): first half =
GSMGIO5BTCPUZZLECHALLENGE (from Decentraland), second half = the prize
address — the URL string itself is "half and better half."

The parcel closure stands — no hidden data, no second entity — but its
description changes: not "one audio cube and decoration," but **a drawn gate
with a descending figure and the key at its base.**

---

# Addendum 42 — the final synthesis: hints × every decrypted text

## Closed this pass (all null, all against live oracles)

The never-tested constructions surfaced by re-reading all hints against all
plaintexts as one document: the phase-2/3 URL tail
(`averyspecialdessertiwroteitmyself` — the Merovingian's cake line) and head
(`choiceisanillusion…` — absent from every prior candidate list); "hash hash"
read as per-phrase hashing joined/seven-digest concatenation; the six-dot
blank's fills (`thefutureisours`…); the passport date in six forms (given
2021-12-31, consumed by nothing, still unconsumed); "knock knock" (the
Wake-up quote's omitted third line); "why / the first question"; "And Oen";
the two-doors passage (deliberately cut from the Architect speech — the
doors were moved out of the text and into the world); dbbi's digit-pairs as
extraction indexes into faed and into the speech (three variants). 624
envelope trials, digest-URL probes, gate probes: **zero.** The archived
phase-2/3 page contains no forms — no second endpoint ever existed
client-side.

## What the cross-reference actually establishes

Every solved joint of this puzzle follows one grammar: **the key is stated
in plaintext adjacent to its lock** (Beaufort named and keyed by the
preamble; the VIC board after its digits; every password from its page's
riddles). The grammar breaks at exactly one place — the terminal envelope —
where the adjacent text is consumed and every literal, transform, recombination
and convention is now measured dead (~7M strings on our side, 2.5B tests on
the corpus's, against self-proving targets). Combined with the creator's own
constraints — *in front of your eyes*, *a true giveaway*, *offline*,
*wordplay not math*, *solvable the same day once seen*, and above all his
one labelled hint: **his close friends, who lack the skills, have the best
chance** — the conclusion is forced rather than suspected:

**The missing key is not derived from puzzle text. It is personal knowledge
— something the creator's friends know about him: his name, a date, a place,
an inside reference.** "In front of your eyes" because it is on his profile,
his company, his chat presence — visible to anyone who knows him, invisible
to text-mining. The passport-date hint is the same shape (trivia, not
ciphers). The prize being "obscure intel" rather than coins completes the
picture: the puzzle is a message from a person, sealed with a person's
reference.

## The ranked remaining possibilities, after this session

1. **The personal key** — identify the creator's real name/public facts
   (company records, the CID-encoded terms PDF, early chat) and test the
   personal family properly.
2. **dbbi/faed's mechanism** — the only unsolved content; every simple
   construction now dead (VIC, Bifid, page-convention, grids, digit-wise
   keys, index-extraction). What remains is a genuinely new reading.
3. **The hint edit-diff** against public 2019–2021 quotes — the texts
   themselves may have changed under us.
4. **A time element** — "the only date I give away," the halving-coupled
   prize, a live server that still validates: the possibility that some part
   is date-gated cannot be excluded from here.

The negative space is now strong enough to state as the session's final
finding: seven years of communal computation and one long day of adversarial
re-reading agree — **what is missing is not more search over the texts. It
is either a person or a reading, and the record now proves which questions
to stop asking.**

---

# Addendum 43 — the poster worked to exhaustion as string-material

The user's direction: work Candidate 1 (the first piece) — the combinations
never run. All families below executed against the live URL/gate/digest
oracles and all three open envelopes (both KDFs, raw + sha256), with the
SalPhaseIon digest construction as a passing live control.

**Measured first:** the nest reads three ways depending on how you look —
center-sample `0100`, corpus `0000`, rabbit-aware `1111` (the line art
crosses all four surplus cells: the nest literally contains the whole
rabbit). All rotations → N ∈ {0,1,2,3,4,15,34,63}.

| family | constructions | result |
|---|---|---|
| nest × marked-char | substitute `n`→N with append/prepend, all N | null (117 strings total across A–C) |
| rose readings | bit-flip (`~`), 25th marker → prime 97, position 163 appended, red-bar values (`ed1c24`, `2372836`), flip+address recipe | null |
| nest-as-index | delete/zero/letter at N in seed and full string; N→marker-prime→apply to seed | null; 1,404 envelope trials, zero pad≥4 |
| **rose applied to other strings** | char 20 / 163 mod L / 12 on the SalPhaseIon input, caption, address, seven-phrase string, checkerboard message — delete/zero/insert | null with passing control |

**The structural conclusion.** The poster's surplus — nest, rose, rabbit —
is now exhausted as a *source of strings*. What it unmistakably still is:
**an instruction set** — mark one character (the rose, at a prime), zero it
out, first-or-zero, reinsert primes. Every operand anyone has tried (the
seed, the caption, the recipe inputs, the phrases, the checkerboard) is dead.
The puzzle teaches operations whose operand is not any known text — which
compresses the remaining space to exactly what Addendum 42 concluded:
the operand is either dbbi/faed (if real) or extra-puzzle knowledge
(personal, dated, or held by the silent finders). The poster is the manual,
not the input.

---

# Addendum 44 — the user's correction: the operand IS in the puzzle

Addendum 42/43 leaned toward an extra-puzzle operand. The user overruled it
on the creator's own words, which are unambiguous and twice-stated: *"No
need. You have all the info"* (#9607) and *"Nope"* to internet-required
(#16624). The close-friends hint means the last step is *simple*, not
*secret*. Correction accepted; the operand hunt continues inside the puzzle.

## The oracle, reexamined from its own source

The SPA's complete API surface, extracted from the 2020 source map: **44
endpoints, all `/api/v1/*`, all platform** (markets, orders, settings,
admin). No puzzle API ever existed client-side. All 44 are dead (404). The
puzzle's live oracle surface is exactly: the four pages, the images, and
`/phase1verification` — which is **case-sensitive** (uppercase of the known
password fails; lowercase passes) and accepts exactly one password known.

## Submitted to the live gate for the first time (all 404)

The remaining in-puzzle operands never before POSTed: phrases 3, 5, 6, 7
individually and all-seven joined; dbbi and faed raw; both zero-reinserted
strings (121, 695) and their sha256s; the cube-coordinate numbers.

## New observation: 91 = 91

The checkerboard message (3.2.2's output) is **91 characters**; dbbi is
**91 letters**. Exact length match between the puzzle's one solved
checkerboard message and its one unsolved letter-field. Two simple pairing
constructions (digit-shift, digit-unshift) produce noise; the match itself
is unexplained — design or coincidence, now on the record.

## Also closed

- The 2023 binary message re-derived from the raw bits: whole-string
  reversal + MSB bytes → **exactly** the seven phrases, 161 bytes, no
  separators, no surplus, no trailing bits. Perfectly accounted.
- Digest-operation family (prime-zeroed / prime-deleted / first-or-zero
  applied to the phase digests as passwords): null.

---

# Addendum 45 — the phase-2 ramble, re-read line by line with Gates in hand

The only solved-stage text with unexplained ornament. Full annotation;
[PART n] = consumed as password part, [EXPLAINED] = decoration accounted
for, [NEW] = reading first made here, [OPEN] = still unexplained.

**"1... are you looking for the private keymaker?"** — [PART 1] The
keymaker is Matrix Reloaded's; "causality" is the Merovingian's doctrine.
**"the ironic 2name of the keymakers… hiding, nearly unprotected, in plain
sight"** — [PART 2] **SafeNet** (Thales). "In plain sight" = the puzzle's
thesis-phrase (recurs as phrase 6).
**"{eps3.4_[in one of the valleys of Phillip]runtime-error.r00., where
daughters hit magic keypads}"** — [EXPLAINED] Mr. Robot S3E4; Angela,
Phillip Price's daughter, at the HSM PIN-pad (community, 2020).
**"When this fails.. Crypto finally to the latin 3Moon?"** — [PART 3]
**Luna**; "to the moon" pun. **"4How so mate?"** — [PART 4] **HSM**.
**"He might have been insecure. His competition might have been that as
well"** (Norton/Thévenin) — [OPEN, weak] ArchOptic sensed it in 2023
("no reason to reference him unless for some other purpose"); likely
flavor reinforcing the security theme.
**"after enough belikins… ruler of a piece of land that's technically the
poorest"** — [EXPLAINED] Belikin = Belize's beer ("drunk"); poorest-by-debt
= USA. **"4 rulers… first name"** (John), **"2 had the firstname in the
surname"** (Johnsons) — [EXPLAINED].
**"One of the rulers had a number, and dirty too"** — [EXPLAINED] the
number = **EO 11110** (part 5's "5binary"); dirty = the conspiracy reading.
**"Another had a resemblance to Carrey, James Gates, also Simulacra and
Simulation"** — [PARTIALLY EXPLAINED + NEW] gnomad (2026) connected
Carrey→Truman Show→simulacra. **Nobody ever worked "James Gates": S. James
Gates Jr.'s famous result is that supersymmetry equations contain
structures identical to error-correcting codes — "the equations of physics
look like computer code."** A code-inside-the-theory-of-everything
reference, planted in 2019, four years before the TOE hint (#8354), beside
Baudrillard and a Truman-Shaw simulacrum: a three-pointer **simulation/
code/universe cluster that pre-figures the puzzle's whole thesis**. The
riddle's ornament is the creator stating his theme.
**"Another ruler was at one point a floating zerg house while being under
control of the dirty one.. too. The one after died too soon."** —
[NEW READING] floating Zerg house = StarCraft's Overlord; the ruler who
*was an Overlord* = **Eisenhower, commander of Operation Overlord**, under
FDR; "the one after [who] died too soon" = **JFK** — fits exactly where the
chat's "Nixon" guess (2024) does not. Cleaner than any community reading;
confirms the paragraph is a president-chain delivering JFK/11110.
**"the idea of this _green_ came _back_"** — [EXPLAINED-ish] greenback
(Jerry 2023; also the Greenback Party); double-reading with green ideas.
**"random magic pieces of metal, that moved in directions that science
couldn't… coming from places where inspiring papers used to be
deposited"** — [NEW READING] coins (metal) from **banks** (where papers
are deposited), moving in directions economics cannot explain: markets.
Flavor framing the chancellor/bailout → Satoshi chain to part 6. Jerry's
gold/silver guess (2023) superseded.
**"Its' raw data after 4 on row 1616"** — [PART 6] genesis scriptSig; the
misplaced apostrophe sits in the creator's confessed-typo class.
**The FEN + "a buddhist is forced to move"** — [PART 7] non-mating move;
buddhist = non-violent.
**"# X 2 S H 4 Y 0 Q B 15 #"** — Q/B/H/S explained; **X, Y [OPEN]**; block
status contested (2021 screenshot vs 2023 "Can't say").
**"Ok kid, on the highway, let put it in the worst gear."** — [OPEN]
Chased since 2020, produced nothing; Addendum 6's flavour precedent
applies.

**Net result:** phase 2 is now explained to within four small residues —
the Gates thesis-marker (now read), X and Y, "insecure," and "worst gear."
The one load-bearing novelty: **the simulation/code cluster (Truman–Gates–
Baudrillard) shows the creator declaring "reality is code" at the puzzle's
surface in 2019** — the same statement the question-mark parcel, the
parity-markers, and the TOE hint all make in their own idioms.

---

# Addendum 46 — dbbi/faed, the three handles run

**1. Powered period scan** (marginal-preserving shuffles, n=300 per period,
periods 2–100). faed: max z = +1.76 (p=49) over 99 tests — the expected max
of that many tests; nothing clears significance. dbbi: max z = +1.77. The
residue worth carrying: dbbi's two highest periods are **exactly its
factors, 11 and 13** (z = +1.77, +1.39), and faed's p=14 (the poster rhythm)
sits at +1.40. Consistent with, not proof of, a transposition-shaped origin
for dbbi at 11×13 or 13×7 — the only 7×13 object in the puzzle.

**2. Channel-difference species** (de-interleaving into k streams, k = all
divisors; reversal; 7×13/13×7 transposition), each stream page-decoded:
noise everywhere (best printable 0.67 on a 6-char artifact).

**3. X2SH as indexing** ("First or zero" resolving base): the block's
values [2, 32, 42, 4, 0, 82, 16, 15] as 0- and 1-based indices into dbbi,
the checkerboard message (91=91), faed, the speech, the phrases, the seed;
negative-H/B variants; adjacency-value variants (X=2, Y=4): no readable
output in any combination.

**Also resolved this pass:** the corpus's "creator-dated 2021-02-12
screenshot" of *"#…# wasn't used"* is **a member's summary of a Discord
Q&A** (#5789, relayed via #5805 "original post from Discord") — not
creator-voice in this export. The only first-hand creator statement on
X2SH is #8569's "Can't say anything about this." X2SH is unspent by
default, the same standing as dbbi: *not used yet ≠ not relevant* (#17688).

## The state of the reading problem

After tonight: dbbi/faed resist periods, static maps, positional maps,
transpositions, de-interleaving, reversal, VIC (raw and zero-reinserted),
the page convention (raw and zero-reinserted), digit-wise running keys,
index-extraction, and pairing with the checkerboard message. The single
live structural hypothesis is the **transposition with shape 11×13/13×7**
(the factor-period elevation), which now needs the right column-order and
zero-placement to be found simultaneously — a wide but finite space, and
the most defined target the strings have ever had.

---

# Addendum 47 — the operator reading, executed in full: every consumable form null

An outside analysis (pasted 2026-09-09) proposed reading the open hints as an
**operator**, not more passphrases: the `#fefefe` cell (254) halved = **127 =
DEL**, "zeroed out" = bit-zeroing (0xFE mask), "First or zero" = zero-based
indexing — applied to the first-piece strings as binary/control-character
mutations, plus a twin-envelope protocol for the two 5-block envelopes. Its
facts check out against primary artifacts (the `enter` run splits the
SalPhaseIon envelope into 64+64 b64 chars = 2+3 blocks exactly; the two
envelopes' salts are `b45a5e3d…` and `3ab585348552415d`). The families below
were then executed against every oracle we have. Self-tested gate (pad≥4 /
pad-16 / printable), control-verified live probes.

## A–D. Password-side (19,104 decrypts, zero hits)

| family | candidates | result |
|---|---:|---|
| **A. byte-mutations as passwords**: 1,127 mutants of 25 base strings (delete, replace/insert 0x7F/0x00, 0xFE/0x7F masks at the marked `n` + indices 5/9/15/19/20/163, whole-string and prime-position low-bit zeroing, halve-byte, flip-bit0, insert/delete `t`) × {raw, sha256hex} × {EVP-SHA256, EVP-MD5} × 3 envelopes | ~17,000 | **0 pads ≥ 4** |
| **B. bespoke "shabef" KDFs**: key=sha(p), iv=sha(p‖salt)/sha(salt‖p), salt-prefixed keys, ct-seeded keys, sha^n iterations (n=2,4,6,8 per *shabefour*/*anstoo*), double-EVP | ~750 | 0 |
| **C. twin-pair protocol**: 17 semantic pairs (479/484, half/betterhalf, first/zero, 254/127, 0xFE/0x7F, seed/mutated-seed, shabef/shabefanstoo…) × concat both ways, sha-of-concat, XOR-of-digests as raw key, cross key₁/iv₂ | ~1,000 | 0 |
| **D. enter-split 2+3**: each half decrypted alone (own EVP IV), second half with chained IV = ct[16:32], pair passwords (k₁→blocks 1–2, k₂→blocks 3–5, both orders) | ~250 | 0 |

## E. The same mutants against the live oracle (controls verified)

- **Door digests**: sha256 of all 246 URL-family byte-mutants → GET `/digest`:
  all 404 "Hello :-)". Control: `/89727c…` → 200 real page.
- **Gate passwords**: all 324 byte-mutants POSTed to `/phase1verification`
  (the byte-level forms Addendum 40's alphanumeric battery never covered):
  all 404. Control: wrong password → 404 "Hello :-)".

The marked-character operation (door #1) is now null in **every consumable
form** — AES password, digest-door, gate password — on a server that does
discriminate. If the operation reading is right, its output is consumed by
something that is none of these.

## F. 91=91: the two-views reading, closed on the simple forms

dbbi (91) × checkerboard message (91, `INCASEYOUMANAGETOCRACKTHIS…`):

1. **Substitution (decisive kill):** if dbbi's digits encoded these letters
   under any per-letter board, each letter must carry one digit. Instead E
   spans 8 of 9 digit values, N spans 5, no digit maps to a unique letter.
   dbbi is **not** a digit-encoding of the message under any simple
   substitution — including every keyed-alphabet board at once.
2. **Vigenère/Beaufort** (dbbi digits 1–9 / 0–8 as running key, both
   directions, ±offset): best variant z = **+0.46** vs 300 digit-shuffles.

## G. The dbbi transposition space — Addendum 46's target, enumerated and killed

30,528 candidates: **all 7! column orders** (13×7), keyed orders for keylen 13
(7 puzzle keys: seed, seed mod 9, dbbi's own digits, message, primes, 479/484),
row-order variants, **spirals CW/CCW on 7×13, 13×7 and the 11×11
zero-reinserted grid** (both write modes) × zero-reinsertion {none, primes
0-based, primes 1-based, pad-to-121} × decodes {bigint→hex, digit-pairs}.
Score = printable-fraction + trigram density, calibrated on 300 digit-shuffles:
**observed max 0.739 vs null-best μ 0.712 σ 0.039 (null max 0.826) — z = +0.68.**
The factor-period elevation of Addendum 46 was another artifact; the
transposition-with-its-own-factors hypothesis is dead.

## H. Coordinates (X2SH + matrixsumlist) on boards

X2SH knowns [2,32,42,4,0,82,16,15] with X,Y swept 0–9 as (row,col)/(col,row)
on the 4-row keyboard and as VIC codes; poster row/column sums as pairs on the
same boards: zero English anywhere (e.g. VIC reading yields `RE.ARROXLLA.AL`).
Dead. (Also re-identified: the 3.2 digit run is the 149-digit VIC ciphertext
that already decodes to the checkerboard message — solved material, no gap.)

## Where this leaves it

The outside analysis's *facts* were right and its *operator framing* was the
correct next thing to execute — but every executable form of it is null. What
survives from it is not mechanical: the hint-wording diff against 2019–2021
public quotes (its Priority 4), which remains a passive-research task. The
state of the reading problem is unchanged from Addendum 46 minus one live
hypothesis: dbbi/faed now also resist their own factors as transposition
shapes, and the 91=91 pairing resists substitution and shift readings.

---

# Addendum 48 — the inventory sentence re-read: selector and operation-count readings, executed

The architect inventory ("SELECT FROM OVER TWENTY THREE CIPHERS SIXTEEN
ENCRYPTIONS AND OR SEVEN INTERTWINED PASSWORDS") was re-read two ways after
the 9-yellow + 7-phrases = 16-char concatenation was judged forced:
(a) **16 = count of AES operations** with the 7 phrases as key material;
(b) **direction reversed** — the 24 markers are the selector, the phrases the
data, 16 the output length.

## Ground truth re-validated first

The marker facts reproduce exactly: 24 markers at spiral positions 7+8k
(7,15,…,191), color bits `F73D92` MSB-first (15 blue, 9 yellow), primes-forward
mapping sums to blue 484 / yellow 479 — zero free parameters. The solver's
"rulhgtwps" reproduces as yellow-prime values, 0-based, into the 161-char
phrase concat. And sweeping all 16 spiral conventions against the recorded
invariant (marker ⇒ (col−row) mod 4 = 1) isolates the puzzle's actual spiral
**uniquely: bottom-right start, CCW** — a new incidental confirmation of the
decode convention.

## The battery (all gated on the free on-chain oracle + the three envelopes)

Every candidate → sha256/sha256²/hex-sha (and raw-32B) → secp256k1 → hash160
vs **both prize addresses** (the first time the selection family has faced the
real prize gate), plus legacy-KDF AES on T32/SALPH/COSMIC with pad≥4:

| family | size | result |
|---|---:|---|
| selectors {all-24 spiral, all-24 primes, yellow-only ×2 forms, blue-only ×2 forms} × sources {phrase concat ±reverse, ybp, checkerboard, speech, seed, path, architect, phrase2, phrase6} × 0/1-based × reductions {full, first16, last16, 9+7} | 240 candidates ≈ 720 key checks, 2,880 decrypts | **0** |
| marker (row,col) coords (validated BR-CCW spiral) as phrase-row/char-col, and as r·14+c / c·14+r / r+c indices into three sources | ~80 | 0 |
| **16-operation cascade**: 7 data seeds (F73D92, marker bits, ybp, seed, sha(URL), concat) × phrase key schedules fwd/rev cycled to 16 ops × {E¹⁶, D¹⁶, alternating, CBC-32} | ~170 | 0 |
| intertwined digests: XOR of the 7 sha256(phrase), interleave-32 | 2 | 0 |
| selectors indexing dbbi/faed (letters as passwords; letters→digits→bigint as raw keys) | 472 | 0 |

Totals ≈ **1,061 prize-key derivations + ~4,200 decrypts, zero hits.**

## Standing

Both new readings of the inventory are null on every oracle available offline;
the concatenation reading was already null (Addendum 36). The sentence's three
numbers have now been consumed as: item counts (closed), an operation count
(closed), and selector/output lengths (closed) — always against the strongest
free gate the puzzle offers. What was *gained*: the unique recovery of the
poster's exact spiral convention, and one more elimination class for the
inventory sentence.
