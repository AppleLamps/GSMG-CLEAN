...
## Addendum 32 — corrections to Addendum 26 and the rabbit layer; the #FEFEFE cell

> **CORRECTED BY ADDENDUM 34.** Its #FEFEFE and Matrix-diff sections re-derive
> Addenda 18 and 16 and wrongly claim novelty. Its "leftover 4 bits = 4" is retracted.


**Retraction (rabbit sub-units).** Earlier notes claim "exactly 62 sub-units depart
from their cell's flat colour". The correct count is **37**, all black-on-white,
bbox rows 30-42 x cols 32-45 of the 70x70 module grid. The layer is *pictorial* —
it draws the rabbit's ears, head and body — not a bitstream. Do not mine it for a payload.

**PNG container is clean.** 1958 bytes; IHDR/sRGB/gAMA/pHYs/IDAT/IEND; all CRCs valid;
0 bytes after IEND; no tEXt/zTXt/iTXt. No appended blob.

**NEW: the fifth colour.** The palette is not 4 colours but 5. `#FEFEFE` occupies
exactly 625 px = one cell-aligned 25x25 block at 14x14 cell (7,4) (module rows 35-39,
cols 20-24). Black cells above and below, pure white left and right. Under the proven
orientation (tr=5, cw spiral, K=1/W=0, B=1/Y=0, off=0) it is **spiral index 163 =
byte 20, bit 3 = the 'n' of "planted"**. Anomalous three ways:
  - only non-K/W cell that is NOT one of the 24 B/Y markers (verified at exactly 7,15,...,191)
  - sits exactly midway between markers 159 and 167
  - unique: symbol counts K=87 W=84 B=15 Y=9 F=1
Meaning undetermined. Strongest remaining candidate for an unextracted image layer.

**Incidental.** 196 cells vs 192 payload bits; the 4 unused cells are the spiral's
centre (where the rabbit is drawn) and read 0100 = 4.

**Supersedes Addendum 26's use of TWENTY-THREE / SIXTEEN / SEVEN.** The clause
"SELECT FROM OVER TWENTY-THREE CIPHERS SIXTEEN ENCRYPTIONS AND OR SEVEN INTERTWINED
PASSWORDS TO FIND THE ACTUAL PRIVATE KEYNOTE" is the Matrix Reloaded Architect line
"select from the Matrix twenty-three individuals, sixteen female, seven male, to
rebuild Zion" with nouns re-skinned (individuals->ciphers, female->encryptions,
male->passwords, rebuild zion->find the actual private keynote; word-level ratio 0.747).
**The numbers are inherited from the film and carry no puzzle information.**
Informative alterations are only those not explained by re-skinning:
  "the source" -> "the source CODES" (insertion, plural)
  "prime program" -> "prime BASICS"
  "private key" -> "private KEYNOTE"

**Sweeps this session, all null, with pad-size instrumentation:**
  SRC1616 (genesis/line-1616 fields)      31,248 decrypts  126 pad-valid vs 122.1  no pad>=4
  SEVEN (5040 perms + 5040 interleaves) 1,088,640 decrypts 4,211 vs 4,252.5        no pad>=4
  H140 (140-char windows, all held text)5,242,860 decrypts 20,482 vs 20,479.9      no pad>=4

**Seventh envelope `74c974e3f92e64b5` — likely NOT creator material.** Byte-identical to
the copy circulating in chat (112 B = 16 hdr + 96 ct, 6 blocks). In cdx.json (403 rows,
2019-04-22 -> 2026-08-30): **none of the six known salts ever appears in a URL** — all six
were delivered as page content. The seventh appears only as a URL path, only in 2026
(20260105015908 full, 20260207190055 truncated prefix). gsmg.io returns 200 on any path;
the same index contains captures of scraped lodash source and HTML-escaped markup as URLs.
Deprioritise.

---

## Addendum 33 — THE FRONTIER: 661 undecoded characters in the SalPhaseIon source

> **CORRECTED BY ADDENDUM 34.** "THE FRONTIER" overstates: the A/B decomposition is
> already at line 794 and `faed` is the group's main anchor (section 1). The alphabet
> diagnostic and the structural measurements stand.


"RETURN TO THE SOURCE CODES" (an *insertion* over the Matrix original "return to the
source" — see Addendum 32) read as: go back to the page source. Doing so pays.

### Structure of the SAL textarea (1075 chars)

    [0:91]     A        91 chars, alphabet a-i        <-- UNDECODED
    [91:195]   binary  104 chars, a/b -> "matrixsumlist"
    [195:765]  B       570 chars, alphabet a-i        <-- UNDECODED
    z
    [766:829]  S1       63 chars, a-i AND o -> "lastwordsbeforearchichoice"
    z
    S2                  29 chars, a-i AND o -> "thispassword"
    z
    "shabef" + "ourfirsthintisyourlastcommand"
    B1 (64 b64 chars) + binary "enter" (40 chars, a/b) + B2 (64 b64 chars)
    "shabef" + "anstoo"

B1+B2 = 96 bytes = the 5-block twin. The embedded word **"enter"** between them is the
creator spelling out the newline of openssl's 64-column wrap. Structural, not a password.

### Why A and B are unsolved, and the diagnostic nobody recorded

The published walkthrough (puzzlehunt/gsmgio-5btc-puzzle README) decodes matrixsumlist,
enter, lastwordsbeforearchichoice and thispassword, then states verbatim:
**"the rest of the text doesn't seem to decode the same way."** A and B are that rest.

**The diagnostic: A and B contain NO 'o'.** Alphabet is exactly a-i.
The two SOLVED segments both contain 'o'. Since the established mapping is
a..i = 1..9 and o = 0, the solved segments are true decimal (0-9) while
A and B are **digits 1-9 with zero never occurring** — the signature of a digital
root or bijective numeration, NOT decimal. That is precisely why
digits -> decimal bigint -> hex -> ASCII produces garbage on them.

### Measured structure

    A: len 91   sum 422   freq a3 b25 c8 d4 e18 f10 g10 h8 i5
       IC=0.1509 (uniform 1/9=0.1111)  chi2 vs uniform = 40.2, df=8  -> genuinely non-uniform
       digram 'be' x10, trigram 'bfb' x3
    B: len 570  sum 3079  freq a54 b49 c52 d49 e69 f57 g107 h58 i75
       IC=0.1181  chi2 = 43.7 (driven by g=107 vs 63 expected)
       per-coset IC flat at every period 1..15 -> no periodicity

### Negative results (do not repeat)

  - base-9 (a=0), base-10 (a=1), bijective-9, on A, B, A+B, B+A, each also reversed:
    all produce byte strings with printable fraction 0.33-0.53 = random. Null.
  - Creator's own attested encoding (from the 2023-02-23 seven-phrases post:
    "each byte is bit-reversed (LSB-first), then the whole string is reversed"),
    applied over every base conversion above: best lowercase fraction 0.194 vs
    0.102 random baseline on 36 bytes = noise. Null.
  - Pair (9x9 Polybius, 81 values) and triple (729) grouping, both offsets,
    IC vs 300 shuffles: every z < 2. B is NOT a Polybius over English. Null.
  - matrixsumlist as literal matrix sums: 14x14 row sums total 102, col sums 102,
    per-colour row sums K=87 W=84 B=15 Y=9. No correspondence to A or B.
    (Windows of B summing to 479/484/963 exist but are chance at 570 digits.)
  - 194 A/B-derived candidates vs the creator's published SHA-256. Null.

### The 2019 "oracle" is spent

`5ac407837447fba24ba2802e4d1e9aecb4580aa29fef1088cc387c180b746f75`
= SHA256("theflowerblossomsthroughwhatseemstobeaconcretesurface"), the PHASE 1
password. It is not an open oracle for later phases. Confirmed by direct match.

### Why this matters

"matrixsumlist" is physically sandwiched BETWEEN A and B. The creator stated the seven
phrases' order but explicitly NOT what each operates on. A phrase sitting between the
two undecoded objects is the strongest available pairing of phrase to object.
A and B are the only objects in SalPhaseIon that no published work has decoded.

---

## Addendum 34 — correcting Addenda 32 and 33; and a sampling hazard in the poster

I re-derived material this file already contained, then presented it as new. Recording
exactly what was already here, so the next session reads before deriving.

### Already documented — my novelty claims withdrawn

| Claimed in Add. 32/33 | Already at |
| --- | --- |
| the `#FEFEFE` cell, cell (7,4), spiral 163, byte 20 bit 3, the `n` of "planted", 163 prime, not a marker, `~` on flip | **Addendum 18**, which also records it as known in chat since 2020 with 179 mentions, and says verbatim "Recorded here so it is not re-found a fourth time" |
| the Architect/Matrix word-diff table | **Addendum 16**, same table, same eight substitutions |
| "23/16/7 are inherited from the film, not a puzzle inventory" | **Addendum 16**, stated as the film's Zion repopulation quota |
| the SAL decomposition 91 + 104 + 570 + 63 + 29 + separators | **line 794**, with the full reconciliation and "No surplus" |
| `5ac407…` = SHA256("theflowerblossoms…"), phase 1 | **line 723** |
| "return to the source codes / prime basics" executed literally | **Addendum 16**, 61,708 decrypts, null |

The A/B strings are not an undiscovered frontier: `dbbi` (91) and `faed` (570) are the
group's primary objects — section 1 of this file debunks the Bifid `btcseed` decode of
`faed` at period 570. Addendum 33's framing is wrong on that point.

### What from 32/33 does survive as new

- **The alphabet diagnostic.** `dbbi` (91) and `faed` (570) use **a–i only, never `o`**,
  while both SOLVED segments (`lastwords`, `thispassword`) do contain `o`. Under the
  established a..i=1..9 / o=0 mapping the solved pair is true decimal and these two are
  digits 1–9 with zero absent — bijective numeration or digital roots, not decimal.
  This is why the published method yields garbage on them. Nothing in this file or the
  public write-ups had recorded it.
- **Structural measurements.** dbbi: IC 0.1509 vs 0.1111 uniform, chi2 40.2 (df=8),
  digram `be` x10. faed: IC 0.1181, chi2 43.7 (driven by g=107 vs 63), per-coset IC flat
  at every period 1–15. Pair (81) and triple (729) grouping vs 300 shuffles: every z < 2,
  so faed is **not** a 9x9 Polybius over English.
- **Negative results** listed in Addendum 33 (base-9/10/bijective families, the creator's
  attested double-reversal, Polybius grouping, matrix sums, 194 candidates vs the oracle).
- **Rabbit deviation layer is 37 modules, not 62**, and is pictorial (Addendum 32).
- **PNG container is clean** — no appended bytes, no text chunks (Addendum 32).
- **Seventh envelope `74c974e3f92e64b5` is probably not creator material** (Addendum 32).
- Sweep nulls: SRC1616 31,248; SEVEN 1,088,640; H140 5,242,860 decrypts, all at chance.

### The matrixsumlist discrepancy, resolved — Addendum 16 was right

Addendum 16 records row sums totalling **101**; I computed 102. Exactly one cell differs,
and it is a **sampling artifact**:

    cell (7,6): centre pixel = K, cell flat colour = W (19 of 25 modules)

The poster's centre pixel for that cell lands on a stroke of the rabbit overlay. Two
consequences:

1. **101 is correct.** matrixsumlist should be read from each cell's FLAT colour, not its
   centre pixel. Correct values:
   rows `[6,10,8,7,6,6,5,4,9,9,7,8,7,9]`  cols `[8,10,8,10,8,7,3,6,7,5,9,6,6,8]`  total 101
2. **The payload is unaffected.** Raw cell (7,6) maps to transformed (6,7) = **spiral
   index 193**, one of the four cells beyond the 192-bit payload. Both samplings decode
   to `gsmg.io/theseedisplanted`.

**Retraction:** Addendum 32's "the 4 unused cells read 0100 = 4" is an artifact of the
same centre-pixel hit. Under flat-colour sampling the tail reads `0000`. The claim meant
nothing and is withdrawn.

**Standing hazard:** `grid14b.py` samples centre pixels and therefore misreads cell (7,6).
It is harmless for the 192-bit payload but wrong for any whole-grid statistic. Any future
grid work must bucket each cell by its flat/majority colour over its 25 modules.

### Address oracle verified, not assumed

`oracle2.py` matches on **HASH160**, not the Base58 string: each target is decoded once
with `b58decode_check` and the version byte stripped, then RIPEMD-160(SHA-256(pubkey)) is
looked up. Both compressed and uncompressed pubkey forms are tried.

    GSMG        1GSMG1JC9wtdSwfwApgj2xcmJPAwx7prBe  a9553269572a317e39f0f518cb87c1a0ee1dbae4
    BetterHalf  17ucy1K9ZUAaoY6JVtM932W9jUp5LXfyHa  4bc468447fe1b048ad030a2f9a125478eabc4ed6

Positive control (a known key registered under both forms) fires; negative control returns
None; hash160 lookup and Base58 string comparison shown equivalent. Both targets are
legacy P2PKH, so there is no P2SH/bech32 gap.

---

## Addendum 35 — Novelty audit against the 2026-09-09 Telegram export

**Question asked:** "Have we discovered anything that the group has not?"

**Corpus:** `D:\rabbitv2\newest-telegram-9.9.26\result.json`, flattened to 56,119 messages,
2019-04-20 → 2026-09-09, 7,319,122 bytes. Every candidate-novel claim from this session
was grepped against it and the *hits were read*, not just counted.

### Result: nothing survives.

| Claim I made | Corpus verdict |
|---|---|
| `#FEFEFE` cell is a distinct 5th palette colour, 625 px | **Known.** 189 hits. Exact palette table incl. `Other White Shade (254,254,254,255): 625 pixels` posted verbatim. Already Addendum 18 here, too. |
| dbbi/faed alphabet a–i has no 'o' → not base 9 | **Known.** msg 12710, 2023-08-30: *"this isn't base 9 as you are eliminating the 0"*. |
| dbbi digit sum = 422 | **Known.** SATE, 2026-08-23: *"dbb = 422"*. |
| matrixsumlist = 101 | **Known.** Pomyk msg 10019, 2023-08-14: *"sum of 1s in the matrix is 101"*. |
| SAL decomposition 91 + 104 + 570 | **Known.** msg 9440, 2023-08-05: *"there is matrixsumlist in between dbbi/faed strings"*. Also already at line 794 of this file. |
| Architect passage is an altered Matrix Reloaded transcript; 23/16/7 are film furniture | **Known.** msg 63629, 2026-05-23: *"The entire passage is altered version of the conversation"*; Sparky links the canonical transcript and identifies the inserted clause. Also already Addendum 16. |
| PKCS#7 pad on the 80-byte twins; pad=16 when plaintext is a 16-multiple | **Known and better developed.** RB, 2024-11-14 (msg 29799) gives the full pad model *and* the 1/256 false-positive rate; repeated 2025-04-14. |
| Plaintext is 64–79 bytes | **Known.** Tobi, msg 29797/29798, 2024-11-14. |
| 64-byte plaintext ⇒ two 32-byte keys ⇒ pad exactly 16 | **Known.** msg 29956, 2024-11-14: *"IMO those are both keys to the two addresses"*; Denis Golovkin 2025-09-02: *"what could be 64 bytes long in a puzzle hiding a 256 bit private key?"*; k1ng 2025-08-08 adds the Bitcoin Core CCrypter layout (32 ‖ 16×0x10 ‖ 32). |
| faed index of coincidence ≈ 0.118, flat, random for 9 symbols | **Known, and already retracted by them.** sniper, 2026-08-11 posts the identical number; B58 replies *"index of coincidence does not apply here"*; sniper concedes: *"base-N encoding of anything, text included, flattens the digit distribution, so IC near random proves nothing."* |
| Exhaustive 9! base-mapping sweep of faed is null | **Known.** sniper, 2026-08-11: *"all 362880 letter→digit permutations in base 9, same in base 10, both directions… zero readable output, best printable ratio 0.52."* |
| hash160 / secp256k1 address oracle | **Known.** 8 `hash160`, 13 `ripemd`, 16 `secp256k1` hits. |

### The one methodological point that stands, and why it is not a discovery

`centre pixel` / `flat colour` / `sampling` return 0–1 hits. Nobody discusses the
cell-(7,6) centre-pixel artifact — because nobody else made that mistake. It was a defect
in `grid14b.py`, not a fact about the puzzle. It is recorded in Addendum 34 as a hazard,
and it should not be counted as a finding.

### The one genuinely unmatched *test*

The Wayback CDX provenance check on all six known salts + `74c974e3f92e64b5` has no
equivalent in the corpus. The closest is an August 2026 request (msg ~69686) whose author
audited *"all five distinct successful Wayback HTML variants"* of the SalPhaseIon page —
page bodies, not salt-in-URL. And `74c974e3f92e64b5` appears **exactly once** in 56,119
messages: inside Kashin Kojin's own list (msg 67688, 2026-07-31). It has no independent
provenance in the group's record either, which corroborates Addendum 32's conclusion:
**probably not creator material.**

### The finding that actually matters

Messages 68455–68475 (2026-08-11) show at least three participants — `sniper`, `B58`, `К` —
running LLM-assisted analysis and reproducing this session's exact experiments within days
of each other, including one participant asking another *"how come you are filtering all
your output through LLM"*. The frontier is not under-analysed. Sweeps of the kind logged in
Addenda 19, 31, 32 and 33 are being independently duplicated by other solvers in real time.

**Operational consequence.** Volume of candidate passwords is not the binding constraint, and
neither is statistical characterisation of dbbi/faed — both are saturated. The only moves with
positive expected value are ones that produce *new material*: an unextracted payload, a
provenance fact, or a password source nobody has enumerated. `yinyang` remains the only one of
the seven creator phrases with no derived value and an operational description from the creator
("It's the next phase"), and it is the only such lead left in this file.
