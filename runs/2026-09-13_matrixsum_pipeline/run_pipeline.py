...
N_CONTROLS = 400
VERSION = "v2 (distinct-pair word score; letter families unchanged from v1)"
INPUTS = {
...
def score_words(ws):
    """v2: number of DISTINCT adjacent pairs of two different words found in Looking Forward bigrams.
    (v1 counted every adjacent pair, so index wrap-around repeats like 'of of of' outscored a planted sentence.)"""
    ws = [re.sub(r'[^a-z]', '', w.lower()) for w in ws]
    return len({(a, b) for a, b in zip(ws, ws[1:]) if a != b and (a, b) in BIGRAMS})
...
def build():
    ...
    man = {
        'version': VERSION, 'v1_script_sha256': '2d51146b96fad902b5e320ee4136c7541f06f266964c2d0afdd0aea9bdf82116',
